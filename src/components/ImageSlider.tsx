...
import { ChevronLeft, ChevronRight, ShieldCheck, Star, Users, Bell } from 'lucide-react';
...
export default function ImageSlider({ emergencyNotice }: ImageSliderProps) {
  ...
  return (
    <div id="hero-slider" className="relative w-full h-[220px] rounded-3xl overflow-hidden cyan-glow-card bg-[#020713]">
      ...
      <div className="absolute inset-0 p-5 flex flex-col justify-between text-left pointer-events-none">
        ...
        <div className="flex items-center space-x-2">
          <span className={`px-2.5 py-1 rounded-full text-[9px] font-mono font-black text-white bg-gradient-to-r ${slides[currentIndex].badgeColor} shadow-md tracking-wider flex items-center gap-1`}>
      ...
      <div className="absolute right-4 bottom-4 flex items-center space-x-1.5 z-10">
        <button
          onClick={prevSlide}
          className="w-7 h-7 rounded-lg bg-black/60 border border-white/10 hover:border-cyan-500/40 text-slate-300 hover:text-white flex items-center justify-center transition active:scale-95 cursor-pointer backdrop-blur"
        ...
        {slides.map((_, idx) => (
          <button
            ...
            className={`h-1.5 rounded-full transition-all duration-300 cursor-pointer ${
              idx === currentIndex ? 'w-5 bg-cyan-400' : 'w-1.5 bg-slate-600'
            }`}
            title={`Slide ${idx + 1}`}
          />
        ))}
